...
class LibraryGUI(QMainWindow):
    ...
    def init_ui(self):
        ...
        # eBook checkbox and URL
        ebook_layout = QVBoxLayout()
        self.ebook_checkbox = QCheckBox('eBook')
        self.ebook_url_input = QLineEdit()
        self.ebook_url_input.setPlaceholderText('eBook URL')
        self.ebook_url_input.setEnabled(False)
        self.ebook_checkbox.stateChanged.connect(self.toggle_ebook_url)
        ebook_layout.addWidget(self.ebook_checkbox)
        ebook_layout.addWidget(self.ebook_url_input)
        input_layout.addLayout(ebook_layout)
        ...
        self.table.setHorizontalHeaderLabels(['Title', 'Author', 'ISBN', 'Type', 'Status', 'URL'])
    ...
    def toggle_ebook_url(self, state):
        self.ebook_url_input.setEnabled(state == Qt.CheckState.Checked.value)
    ...
    def add_book(self):
        ...
        ebook_url = self.ebook_url_input.text().strip() if is_ebook else ""

        if not all([title, author, isbn]):
            QMessageBox.warning(self, 'Error', 'Please fill in all required fields')
            return

        if is_ebook and not ebook_url:
            QMessageBox.warning(self, 'Error', 'Please provide eBook URL')
            return
        ...
        self.table.setItem(row, 3, QTableWidgetItem('eBook' if is_ebook else 'Physical'))
        self.table.setItem(row, 4, QTableWidgetItem('Available'))
        self.table.setItem(row, 5, QTableWidgetItem(ebook_url))
